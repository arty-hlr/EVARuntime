...
| Condition M2 | État |
|---|---|
| Runtime installé et vérifié | `[~]` — AUT-016 livré et testé : extraction défensive, bascule atomique, version relue sur le binaire posé, fail-closed. **AUT-018** (2026-08-03) ferme le blocage de configuration : `--runtime-variants` rend la matrice épinglée fournissable par l'opérateur, et une URL de page de releases est refusée avant le téléchargement. Reste ouvert ce que le code ne peut pas produire : **aucune empreinte amont réelle n'a été relevée dans ce dépôt**, donc aucune installation de runtime n'a encore été exécutée pour de vrai |
